...
No hace falta instalar nada. Abrí en el navegador del celular:

```
https://iphone-connection.vercel.app/auditoria
```

Muestra las 256 imágenes con referencia, marca, color, capacidad, estado, batería,
disponibilidad y precio. Filtrá por categoría o tocá "Solo generadas" para ver qué
productos siguen sin fotografía real.

La vista densa muestra 12 por fila: sirve para barrer todo el catálogo en dos minutos.

**Playwright no se usa.** No es compatible con Android (`Unsupported platform: android`).
...
```bash
cd ~/proyecto && npm run doctor
```

Detecta que estás en Termux y te dice el comando exacto para instalar lo que falte.

Para trabajar sólo hace falta **Node**:

```bash
pkg install -y nodejs-lts git
```

Python es opcional: sólo para reprocesar la planilla y regenerar imágenes.

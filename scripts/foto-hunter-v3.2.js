...
function usarUltimaDescarga(slug, callback) {
  if (!fs.existsSync(DOWNLOAD_DIR)) {
    return callback(new Error('No se encontro la carpeta ' + DOWNLOAD_DIR));
  }
  ...
  const files = fs.readdirSync(DOWNLOAD_DIR)
    .map(f => {
      ...
      try {
        ...
        return { nombre: f, ruta: ruta, mtime: stat.mtime.getTime(), size: stat.size };
      } catch (e) {
        return null;
      }
    })
  ...
  if (files.length === 0) {
    return callback(new Error('No se encontraron archivos recientes en ' + DOWNLOAD_DIR));
  }
  ...
  if (!encontrado) {
    return callback(new Error('No se encontro ninguna imagen reciente en ' + DOWNLOAD_DIR + '. Asegurate de haber descargado una foto primero.'));
  }
  ...
  const modelosHtml = modelos.map(m => {
    ...
    <div class="modelo ${doneClass}" data-slug="${m.slug}" data-cat="${m.categoria}" data-query="${qEsc}">
      ...
      <div class="download-box">
        <button class="btn-auto" id="btn-auto-${m.slug}" onclick="usarUltimaDescarga('${m.slug}')">
        ...
        <div class="url-row">
          <input type="text" class="url-input" id="url-${m.slug}" placeholder="URL directa de la imagen..." oninput="previewImage('${m.slug}')">
        ...
        <button class="btn-download" id="btn-dl-${m.slug}" onclick="descargar('${m.slug}')" disabled>Descargar desde URL</button>
      </div>
...
<body>
<h1>Foto Hunter v3.2</h1>
<p class="sub">Busca la foto, descargala normalmente, volve y toca "Usar ultima descarga"</p>

<div class="server-info">Servidor activo en <b>http://localhost:${PORT}</b><br><small>Detecta automaticamente la ultima foto de /sdcard/Download/</small></div>
...
<script>
const LS_KEY = 'ic_foto_hunter_v32';
...
console.log('\\nFoto Hunter v3.2 (Auto-Detectar Descargas)\\n');
      ...
      try {
        ...
        usarUltimaDescarga(slug, (err, result) => {
          if (err) {
            ...
            console.log('Guardado: ' + path.basename(result.path) + ' (' + result.sizeKb + ' KB) desde ' + result.origen);
        ...
        const req2 = client.get(url, {
          ...
          if ([301, 302, 303, 307, 308].includes(res2.statusCode)) {
            if (!res2.headers.location) {
              res.writeHead(500, { 'Content-Type': 'application/json' });
              return res.end(JSON.stringify({ error: 'Redirect sin Location' }));
            }
            return;
          }
          if (res2.statusCode !== 200) {
            res.writeHead(500, { 'Content-Type': 'application/json' });
            return res.end(JSON.stringify({ error: 'HTTP ' + res2.statusCode }));
          }

          const ct = (res2.headers['content-type'] || '').toLowerCase();
          if (ct.includes('text/html')) {
            res.writeHead(500, { 'Content-Type': 'application/json' });
            return res.end(JSON.stringify({ error: 'La URL devuelve una pagina web, no una imagen' }));
          }
          ...
          file.on('finish', () => {
            ...
            const stats = fs.statSync(destino);
            res.writeHead(200, { 'Content-Type': 'application/json' });
            res.end(JSON.stringify({ success: true, sizeKb: Math.round(stats.size / 1024), path: destino }));
          });
          file.on('error', (err) => {
            fs.unlink(destino, () => {});
            res.writeHead(500, { 'Content-Type': 'application/json' });
...
server.listen(PORT, '0.0.0.0', () => {
  ...
  console.log('   2. Toca Google para buscar la foto del modelo');
  console.log('   3. Descarga la imagen normalmente (va a /sdcard/Download/)');
  console.log('   4. Volves a Foto Hunter y toca "Usar ultima descarga"');
  console.log('   5. Listo. La foto ya esta en public/maestras/ con el nombre correcto.');

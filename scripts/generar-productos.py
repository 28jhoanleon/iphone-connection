...
def aclarar(hex_color: str, k: float) -> str:
    """Versión más clara de un color, para el degradado."""
    n = int(hex_color[1:], 16)
    r, g, b = (n >> 16) & 255, (n >> 8) & 255, n & 255
    f = lambda v: min(255, int(v + (255 - v) * k))
    return f"#{f(r):02X}{f(g):02X}{f(b):02X}"


def defs(c: str, bd: str) -> str:
    """
    Degradado, reflejo y pantalla compartidos.
    Dan volumen sin filtros: el render deja de leerse como icono plano y se
    acerca al aspecto de una fotografía de estudio.
    """
    return (
        '<defs>'
        f'<linearGradient id="cuerpo" x1="0" y1="0" x2="1" y2="1">'
        f'<stop offset="0" stop-color="{aclarar(c, 0.16)}"/>'
        f'<stop offset="0.52" stop-color="{c}"/>'
        f'<stop offset="1" stop-color="{bd}"/>'
        '</linearGradient>'
        '<linearGradient id="brillo" x1="0" y1="0" x2="1" y2="1">'
        '<stop offset="0" stop-color="#FFFFFF" stop-opacity=".28"/>'
        '<stop offset="0.42" stop-color="#FFFFFF" stop-opacity="0"/>'
        '</linearGradient>'
        '<linearGradient id="pantalla" x1="0" y1="0" x2="0.7" y2="1">'
...
def telefono(p):
    ...
    partes = [f'<rect x="{x}" y="{y}" width="{w}" height="{h}" rx="{r}" fill="url(#cuerpo)" stroke="{bd}" stroke-width="1.5"/>']

    if n == 0:  # SE 2 · marcos gruesos y botón de inicio
        partes.append(f'<rect x="{x+14}" y="{y+68}" width="{w-28}" height="{h-142}" fill="url(#pantalla)"/>')
        ...
        partes.append(f'<rect x="{x+16}" y="{y+16}" width="{w-32}" height="{h-24}" rx="{r-10}" fill="url(#pantalla)"/>')
    ...
    partes.append(f'<rect x="{x}" y="{y}" width="{w}" height="{h}" rx="{r}" fill="url(#brillo)"/>')
    return "".join(partes)
...
def reloj(p):
    ...
    x, y = 400 - w/2, 400 - h/2
    return (defs(c, bd) + f'<rect x="{400-63}" y="{y-140}" width="126" height="158" rx="32" fill="{bd}" opacity=".45"/>'
            f'<rect x="{400-63}" y="{y+h-18}" width="126" height="158" rx="32" fill="{bd}" opacity=".45"/>'
            f'<rect x="{x}" y="{y}" width="{w}" height="{h}" rx="{60 if ultra else 54}" fill="url(#cuerpo)" stroke="{bd}" stroke-width="2"/>'
...
def tablet(p):
    ...
    w, h = 330, 452
    x, y = 400 - w / 2, 400 - h / 2
    return (defs(c, bd) + f'<ellipse cx="400" cy="{400+h/2+26}" rx="{w*0.46}" ry="15" fill="#000" opacity=".07"/>'
            f'<rect x="{x}" y="{y}" width="{w}" height="{h}" rx="26" fill="url(#cuerpo)" stroke="{bd}" stroke-width="2"/>'
...
def notebook(p):
    c = color_de(p); bd = sombra(c)
    return (defs(c, bd) + f'<ellipse cx="400" cy="520" rx="230" ry="14" fill="#000" opacity=".07"/>'
            f'<rect x="228" y="228" width="344" height="230" rx="12" fill="url(#cuerpo)" stroke="{bd}" stroke-width="2"/>'
...
def parlante(p):
    c = color_de(p); bd = sombra(c)
    return (defs(c, bd) + f'<ellipse cx="400" cy="546" rx="170" ry="14" fill="#000" opacity=".07"/>'
            f'<rect x="252" y="258" width="296" height="278" rx="46" fill="url(#cuerpo)" stroke="{bd}" stroke-width="2"/>'
...
def camara(p):
    c = color_de(p); bd = sombra(c)
    return (defs(c, bd) + f'<ellipse cx="400" cy="530" rx="150" ry="13" fill="#000" opacity=".07"/>'
            f'<rect x="272" y="282" width="256" height="230" rx="34" fill="url(#cuerpo)" stroke="{bd}" stroke-width="2"/>'
...
def cable(p):
    c = color_de(p); bd = sombra(c)
    return (defs(c, bd) + f'<path d="M300 250 q-70 150 0 300" fill="none" stroke="{c}" stroke-width="22" stroke-linecap="round"/>'
...
def cargador(p):
    c = color_de(p); bd = sombra(c)
    return (defs(c, bd) + f'<ellipse cx="400" cy="524" rx="120" ry="12" fill="#000" opacity=".07"/>'
            f'<rect x="300" y="292" width="200" height="216" rx="40" fill="url(#cuerpo)" stroke="{bd}" stroke-width="2"/>'
...
def joystick(p):
    c = color_de(p); bd = sombra(c)
    return (defs(c, bd) + f'<ellipse cx="400" cy="512" rx="160" ry="13" fill="#000" opacity=".07"/>'
            ...
            f'fill="url(#cuerpo)" stroke="{bd}" stroke-width="2"/>'
...
def auriculares_in(p):
    ...
    c = color_de(p); bd = sombra(c)
    return (defs(c, bd) + f'<rect x="292" y="352" width="216" height="176" rx="52" fill="url(#cuerpo)" stroke="{bd}" stroke-width="2"/>'
            f'<path d="M292 402 h216" stroke="{bd}" stroke-width="2" opacity=".5" fill="none"/>'
            f'<path d="M340 352 v-92 a26 26 0 0 1 52 0 v92" fill="url(#cuerpo)" stroke="{bd}" stroke-width="2"/>'
            f'<path d="M408 352 v-92 a26 26 0 0 1 52 0 v92" fill="url(#cuerpo)" stroke="{bd}" stroke-width="2"/>'
...
def auriculares_over(p):
    ...
    c = color_de(p); bd = sombra(c)
    return (defs(c, bd) + f'<path d="M284 404 a116 116 0 0 1 232 0" fill="none" stroke="{c}" stroke-width="30" stroke-linecap="round"/>'
            f'<path d="M284 404 a116 116 0 0 1 232 0" fill="none" stroke="{bd}" stroke-width="30" stroke-linecap="round" opacity=".25"/>'
            f'<rect x="244" y="384" width="86" height="150" rx="38" fill="url(#cuerpo)" stroke="{bd}" stroke-width="2"/>'
            f'<rect x="470" y="384" width="86" height="150" rx="38" fill="url(#cuerpo)" stroke="{bd}" stroke-width="2"/>'
...
def consola(p):
    ...
    c = color_de(p); bd = sombra(c)
    return (defs(c, bd) + f'<rect x="286" y="236" width="150" height="404" rx="20" fill="url(#cuerpo)" stroke="{bd}" stroke-width="2"/>'
            ...
            f'<rect x="470" y="470" width="128" height="86" rx="34" fill="url(#cuerpo)" stroke="{bd}" stroke-width="2"/>'
...
def escapar(t: str) -> str:
    """Los nombres traen comillas de pulgadas (MacBook 13\") que rompían el XML."""
...
def svg(p):
    ...
    return (f'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 800 800" width="800" height="800" '
            f'role="img" aria-label="{escapar(p["nombre"])}"><rect width="800" height="800" fill="{BG}"/>'

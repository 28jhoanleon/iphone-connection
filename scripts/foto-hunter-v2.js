#!/usr/bin/env node
/**
 * foto-hunter-v2.js — iPhoneConnection Foto Hunter (desde cero)
 * 
 * Muestra TODOS los modelos del catalogo para descargar fotos desde cero.
 * Uso: node scripts/foto-hunter-v2.js
 */
...
console.log('\n📸 iPhoneConnection — Foto Hunter v2 (desde cero)\n');
...
const todos = [...modelos.values()];
console.log('📊 Total de modelos en catalogo: ' + todos.length);
console.log('────────────────────────────────────────\n');

let cats = [...new Set(todos.map(m=>m.categoria))].sort();
let catOptions = cats.map(c => '<option value="' + c + '">' + c + '</option>').join('');

let modelosHtml = todos.map(m => {
...
<title>📸 iPhoneConnection — Foto Hunter v2</title>
<style>
...
<body>
<h1>📸 Foto Hunter v2 — Desde Cero</h1>
<p class="sub">Todos los modelos. Toca el motor de busqueda, descarga la foto oficial y marca como listo.</p>
...
<script>
const LS_KEY = 'ic_foto_hunter_v2';
...
function resetAll(){
  if(!confirm('¿Borrar TODO el progreso? Se perderan las marcas de listo.')) return;
  state = {};
  save(state);
  updateUI();
}
...
console.log('   O servilo: npx serve public');

// -- Resumen final --------------------------------------------------------
console.log('\n══════════════════════════════════════════════════');
console.log('RESUMEN DEL CATALOGO COMPLETO:');
console.log('══════════════════════════════════════════════════');
const porCat = {};
...
console.log('══════════════════════════════════════════════════');
console.log('\n👉 Proximo paso:');
console.log('   1. Copia foto-hunter-v2.js a scripts/ de tu proyecto');
console.log('   2. cd ~/proyecto && node scripts/foto-hunter-v2.js');

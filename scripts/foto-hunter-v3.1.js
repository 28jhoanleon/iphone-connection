#!/usr/bin/env node
/**
 * foto-hunter-v3.1.js — Con validacion de imagen real
 * Rechaza automaticamente URLs que devuelvan HTML en vez de imagen
 */
...
function descargarImagen(url, slug, callback) {
  ...
  const req = client.get(url, {
    ...
    // Rechazar si el Content-Type dice que es HTML
    if (ct.includes('text/html') || ct.includes('application/xhtml')) {
      return callback(new Error('La URL devuelve una pagina web, no una imagen. Usa "Copiar URL de imagen" en vez de copiar el link de la pagina.'));
    }
      ...
      if (!validado && primerosBytes.length < 16) {
        ...
        if (primerosBytes.length >= 4) {
          if (!esImagenReal(primerosBytes)) {
            ...
            req.destroy();
            validado = true;
            return callback(new Error('El archivo no es una imagen real. Probablemente sea una pagina web. Usa "Copiar URL de imagen" desde Google Images.'));
          }
          validado = true;
        }
      }
  ...
  const modelosHtml = modelos.map(m => {
    ...
    <div class="modelo ${doneClass}" data-slug="${m.slug}" data-cat="${m.categoria}" data-query="${qEsc}">
      ...
      <div class="download-box">
        <div class="url-row">
          <input type="text" class="url-input" id="url-${m.slug}" placeholder="Pega aca la URL DIRECTA de la imagen (termina en .jpg/.png)..." oninput="previewImage('${m.slug}')">
...
<body>
<h1>Foto Hunter v3.1</h1>
<p class="sub">Busca la foto, copia la URL DIRECTA, pegala aca y descarga. Va directo a public/maestras/</p>

<div class="server-info">Servidor activo en <b>http://localhost:${PORT}</b><br><small>Valida automaticamente que sea una imagen real</small></div>
...
<script>
const LS_KEY = 'ic_foto_hunter_v31';
...
console.log('\nFoto Hunter v3.1 (con validacion de imagen real)\n');
...
server.listen(PORT, '0.0.0.0', () => {
  ...
  console.log('\nIMPORTANTE: Ahora valida automaticamente que la URL sea una imagen real.');
  console.log('Si pegas una URL de pagina web, te avisa y no la guarda.\n');
  console.log('Proximo paso:');
  console.log('   1. Abri Chrome y anda a http://localhost:' + PORT);
  console.log('   2. Busca la foto en Google Images');
  console.log('   3. MANTENE PRESIONADA la imagen -> "Copiar URL de imagen"');
  console.log('   4. Pegala en el campo del modelo y toca "Descargar y guardar"');

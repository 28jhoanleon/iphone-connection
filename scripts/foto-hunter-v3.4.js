#!/usr/bin/env node
/**
 * foto-hunter-v3.4.js — iPhoneConnection Foto Hunter (Colores combinados)
 * 
 * Ahora detecta TODOS los colores (tanto los arrays "colores" como los strings "color")
 * para que no falte ninguna opción en la búsqueda.
 */
...
const IMAGE_MAGIC = { jpg: Buffer.from([0xFF, 0xD8]), png: Buffer.from([0x89, 0x50]), gif: Buffer.from([0x47, 0x49]), webp: Buffer.from([0x52, 0x49]) };

function normalizarColor(cadena) {
  if (!cadena) return '';
  return cadena.toLowerCase().replace(/ /g, '-').normalize('NFD').replace(/[\u0300-\u036f]/g, '');
}
...
function generarHtml(modelos, yaDescargados) {
  ...
  const modelosHtml = modelos.map(m => {
    // AHORA COMBINAMOS TODOS LOS COLORES
    let coloresList = [];
...
<title>Foto Hunter v3.4 - Colores Total</title>
<style>
...
<body>
<h1>Foto Hunter v3.4 - Colores Total</h1>
<p class="sub">Ahora aparecen TODOS los colores de cada modelo, incluso los que estaban ocultos.</p>
...
<script>
const LS_KEY = 'ic_foto_hunter_v34';
...
console.log('\nFoto Hunter v3.4 (Combinador de Colores)\n');
...
for (const item of catalogo) {
  ...
  if (!modelosMap.has(key)) {
    modelosMap.set(key, { slug: key, modelo: item.modelo, marca: item.marca, categoria: item.categoria, refs: [], unidades: 0, colores: [], color: null });
  }
  const m = modelosMap.get(key);
  m.refs.push(item.ref);
  m.unidades++;
  // ¡Lógica de colores combinada!
  if (item.colores && Array.isArray(item.colores)) {
    for (const col of item.colores) {
      if (!m.colores.includes(col)) m.colores.push(col);
    }
  } else if (item.color) {
    if (!m.colores.includes(item.color)) m.colores.push(item.color);
  }
}
    ...
    req.on('end', () => {
      try {
        ...
        usarUltimaDescarga(slug, (err, result) => {
          ...
          else { res.writeHead(200, { 'Content-Type': 'application/json' }); res.end(JSON.stringify({ success: true, sizeKb: result.sizeKb, path: result.path, origen: result.origen })); }
        });
...
server.listen(PORT, '0.0.0.0', () => {
  ...
  console.log('\n   ✅ AHORA VERAS TODOS LOS COLORES EN EL LISTADO.');
  console.log('   Busca el Naranja, el Azul, el Negro, el Blanco que te faltaban.');
  console.log('   Descarga sus fotos, usa el script de Python, y despliega.');
});

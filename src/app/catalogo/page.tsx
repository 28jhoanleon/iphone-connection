...
export default function CatalogoPage() {
  ...
  // Estado para controlar los Acordeones (qué categorías están abiertas)
  ...
  return (
    ...
    >
      <div className="max-w-6xl mx-auto">
        ...
        {/* BOTONES DE CONTROL DE ACORDEÓN */}
        {hasProducts && (
          <div className="flex justify-end gap-3 mb-6">
            <button onClick={expandAll} className="text-xs text-gray-600 font-medium px-3 py-1 bg-gray-200 rounded-full hover:bg-gray-300 transition-colors">
              Expandir todo
            </button>
            <button onClick={collapseAll} className="text-xs text-gray-600 font-medium px-3 py-1 bg-gray-200 rounded-full hover:bg-gray-300 transition-colors">
              Contraer todo
            </button>
          </div>
        )}

        {/* CATÁLOGO CON ACORDEONES */}
              ...
              {categoryNames.map((category) => {
                ...
                return (
                  <motion.div 
                    ...
                    layout
                  >
                    {/* CABECERA DE LA CATEGORÍA (El botón del Acordeón) */}
                    <button 
                      onClick={() => toggleCategory(category)}
                      className="w-full flex justify-between items-center p-5 bg-white hover:bg-gray-50 transition-colors text-left border-b border-gray-100/0 focus:outline-none"
                    >
                      <h2 className="text-xl font-semibold text-gray-800">
                        {category} <span className="text-sm font-normal text-gray-400 ml-2">({items.length})</span>
                      </h2>
                      <motion.div 
                    ...
                    {/* CUERPO DE LA CATEGORÍA (Contenido expandible) */}
                                  ...
                                  <div className="mt-auto border-t border-gray-100 pt-3 space-y-1 text-xs text-gray-600">
                                    {product.capacidadGb && (
                                      <div className="flex justify-between">
                                        <span>Capacidad:</span>
                                        <span className="font-medium text-gray-900">{product.capacidadGb} GB</span>
                                      </div>
                                    )}
                                    {product.color && (
                                      <div className="flex justify-between">
                                    ...
                                    {product.config && (
                                      <div className="flex justify-between">
                                        <span>Config:</span>
                                        <span className="font-medium text-gray-900 truncate max-w-[150px]">{product.config}</span>
                                      </div>
                                    )}
                                    {!product.config && !product.capacidadGb && !product.color && (
                                      <div className="text-center text-gray-400 italic py-1">Sin especificaciones</div>
                                    )}
                                  </div>
        ...
        <div className="mt-16 text-center border-t border-gray-200 pt-8">
          <p className="text-xs text-gray-400 max-w-2xl mx-auto leading-relaxed">
            ...
            El producto final puede variar ligeramente. Consulte siempre por WhatsApp antes de comprar.
          </p>
        </div>
      </div>
    </motion.div>
